...
優先順の考え方は共通である。W8 判定ダンプ＋振動キャプチャ＋色確認＋PLAYER_INFO 確認を 1 回の HW 占有で回収し、机上側で U3/U5 確定・rumble 連鎖 unpark・最終レビューに進める (`docs/history/2026-09-15-wdt/hw-batch-2026-09-15.md:1-4`)。

## 1. rumble 連鎖（振動キャプチャ待ち）

> 🚧 In-progress: 振幅復号式は実測待ちであり、式の捏造は禁止である。`0x10` 出力 8B の layout 特定が spike であり、非ゼロ `A2 10` 行を 1 行以上回収できれば Task 3→4→5→6→9→最終レビューへ即進行可である (`docs/superpowers/specs/2026-09-15-uart-features-design.md:28-31,60`, `docs/superpowers/plans/2026-09-15-plan-a-telemetry-v4.md:98-116`, `docs/history/2026-09-15-wdt/hw-batch-2026-09-15.md:20-24`)。
...
## 2. spec v4 改訂（Plan A Task 9、文書のみ）

> 🚧 In-progress: コードは v4（`PROTO_VER=0x04`、`T_PLAYER_INFO` 追加、HELLO v4 のみ）まで進んだが、仕様書は v3 のままである。改訂は Task 9 の未着手項目である (`docs/superpowers/plans/2026-09-15-plan-a-telemetry-v4.md:384-404`)。
...
> 🚧 In-progress: Sub-project B は机上設計までであり、コード＋ビルド以降は未着手である。順序は A→B→C であり、A は host-test 可で HW 不要、B はコード＋ビルドまで机上、C は spike 棚卸しからである (`docs/superpowers/specs/2026-09-15-uart-features-design.md:11`)。
...
- W8 判定ダンプ（U3・U5 確定用、最優先）：`log/switch-bcon-w8-epoch.uf2` を flash し、通常ペア→open（host 保存）→約 2 秒後 WDT 死を 1 回再現し、次 boot の `w8epoch`〜`w8ev` 行一式を回収する (`docs/history/2026-09-15-wdt/hw-batch-2026-09-15.md:11-17`)
