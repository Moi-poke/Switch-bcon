...
- RUMBLE：Pico→PC の振動振幅（`0x22`、LEN2）。現状は予約・未送出であり、STATUS bit7 の計数のみが生きている (`spec/protocol_v3.md:152-154`, `src/bt/hid.c:424-431`)
- PLAYER_INFO：Pico→PC のランプ＋フラグ（`0x23`、LEN2）。dispatch＋配線済み、HW 裏取り待ちである (`src/proto/dispatch.h:58-61`, `src/proto/dispatch.c:103-114`)
