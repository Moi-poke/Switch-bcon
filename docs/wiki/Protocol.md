...
合計は `N + 5`（`N`=ペイロード長、上限 32）である (`spec/protocol_v3.md:39`)：

| Offset | Field | Size | 説明 |
|---|---|---|---|
| 0 | SYNC | 1 | 固定 `0xAB` (`src/proto/protocol.h:12`) |
| 1 | TYPE | 1 | 種別（下表） |
| 2 | LEN | 1 | ペイロード長。既知型は正確長を強制 (`src/proto/protocol.c:86-101`) |
| 3 | PAYLOAD | N | TYPE 依存 |
| 3+N | SEQ | 1 | 方向独立・mod256。送信ごとに +1 (`spec/protocol_v3.md:47`) |
...
- CRC-8/SMBUS（poly `0x07`、init `0x00`、refin/refout なし、xorout `0x00`）。検査値 `crc8("123456789")=0xF4` (`spec/protocol_v3.md:198`)。実装はテーブル駆動である (`src/proto/protocol.c:5-29`)
- SYNC はペイロード中にも出現し得る。最終判定は必ず CRC で行う (`spec/protocol_v3.md:51`)
- 蓄積バッファ上で反復処理する。先頭 `0xAB` 整列→LEN 検証（既知型は正確長）→CRC 検証の順であり、不一致なら ERR 加算＋先頭 1B 前進（フレーム全体を捨てない）(`spec/protocol_v3.md:203`, `src/proto/protocol.c:79-130`)
- 未知型は 32B 上限で可変スキップする（前方互換）(`spec/protocol_v3.md:52`, `src/proto/protocol.c:81-84`)
...
- HELLO ペイロードは `[0]=PROTO_VER（要求版）、[1]=FLAGS（bit0=STATUS 自動送信要求、他予約 0）` である (`spec/protocol_v3.md:126`)
- 現コードは v4 のみ受け付ける。`ver == PROTO_VER`（=4）なら `RESULT_OK` + `state_accept=true`、それ以外は `RESULT_VER_UNSUPPORTED` + `state_accept=false` である。`DOWNGRADED` 分岐は削除済みである (`src/proto/dispatch.c:56-59`)。`DOWNGRADED` 列挙値自体は `protocol.h` に残置されているが無害な残骸であり、計画書も削除不要としている (`src/proto/protocol.h:39-43`, `docs/superpowers/plans/2026-09-15-plan-a-telemetry-v4.md:79`)
- HELLO_ACK ペイロードは `[0]=採用版、[1]=FW_MAJOR、[2]=FW_MINOR、[3]=RESULT` である (`spec/protocol_v3.md:127`)。送信は `flush_outbox` が行う (`src/main.c:839-866`)。FW 版は `FW_MAJOR=0`、`FW_MINOR=1` である (`src/proto/dispatch.h:13-14`)
- v3 HELLO は UNSUPPORTED になる。STATE は拒否されるが NEUTRAL は安全停止として通す (`src/proto/dispatch.c:34-40`, `tests/host/test_config.c:36-48`)
- HELLO なし既定では STATE を受け付ける（sweep/PoC 互換のため `state_accept` 初期値は true）(`src/proto/dispatch.c:7-13`, `tests/host/test_config.c:50-56`)
- PC 送信側は版バイト 4 で送る (`src/poc_dualcore/poc_send.py:102-104`)。`--hello` は HELLO_ACK＋自動 STATUS を確認する (`src/poc_dualcore/poc_send.py:164-182`)
...
ERRCODE は `0x00` 正常／`0x01` LEN 不正／`0x02` CRC 不一致／`0x03` SEQ 欠番／`0x04` UNSUPPORTED 版／`0x05` UART overrun／`0x06` parser overflow／`0x10-0x1F` CONFIG 拒否（`0x10+TYPE 下位`）である (`spec/protocol_v3.md:144`)。CONFIG 拒否コードは `cfg_err` が作る (`src/proto/dispatch.c:25-28`)。周期送信時の errcode は直近エラーを保持し、正常復帰後に 0 へ戻す (`spec/protocol_v3.md:145`, `src/main.c:1018-1021`)。
...
- PING（LEN0）に対し、PONG（LEN1）は受信 PING のヘッダ SEQ をエコーする（RTT 対応を一意に）(`spec/protocol_v3.md:122`, `src/proto/dispatch.c:41-43`, `src/main.c:844-845`)。ホスト試験は `0xAB` エコーを確認する (`tests/host/test_config.c:58-64`)
- NEUTRAL（LEN0）は BTN=0・全スティック `0x80` を即時適用する。STATE 全中立と等価の短縮形である (`spec/protocol_v3.md:118`)。UNSUPPORTED 下でも安全停止は通す (`src/proto/dispatch.c:39-40`, `src/main.c:306-313`)

## タイミング・安全

STATE 送信は変化時即送＋定期リフレッシュ（既定 60Hz、上限 1kHz）。USB ポーリングは bInterval 8（実機写し）で、低遅延は到着即 report 更新で確保する。timeout-neutral は直近 STATE から 200ms 無受信で全解放する。WDT は 2s であり、timeout-neutral とは別機構（前者=入力安全、後者=生存性）である。USB 監視は unmount/suspend で即中立、再 mount で復帰する (`spec/protocol_v3.md:213`, `src/main.c:61,1022-1042`)。
...
- USB ControllerData 12B は timer・電池 `0x91`・ボタン3B（`btn[1]|=0x80`・`btn[2]&=0xCF`）・スティック6B・振動 `0x09` である (`src/usb/usb_hid.c:56-68`)。`0x30` 64B は ID + 12B + 36B IMU(0) + 15B 埋めである (`src/usb/usb_hid.c:70-79`, `src/usb/usb_hid.h:33-35`)。IMU無効時は 0 のままにする (`src/usb/usb_hid.h:33-35`)。マスクの期待値は host 試験 `[6]` が縛る (`tests/host/test_usb.c:165-178`)
- USB `0x21` 64B は ID + 12B 状態 + ack/sub + 本文である (`src/usb/usb_hid.c:140-143`)。`0x01` 対 `01/02/03` の雛形は 2wiCC `procon_data.c` の実働値であり type 1 のみ自 MAC を ASCII 埋めする (`src/usb/usb_hid.c:81-91,144-161`)。`0x02` 本文は `03 48 03 02 + MAC6 + 01 02` であり末尾 `02` が SPI 色有効の条件である (`src/usb/usb_hid.c:162-175`)。`80 xx` 応答は常に 64B ゼロパディングで返し `81 01 00 <type=0x03> <mac6>` を含む (`src/usb/usb_hid.c:19-54`)
- BT `0x30` 14B は `A1 30 timer 80 btn3 stick6 08` であり (`src/bt/hid.c:161-174`)、応答共通部 16B は `A1 21 timer 80 btn3 stick6 08 ack sub` である (`src/bt/hid.c:136-151`)。応答は `A1+ID+本体48=50B` へ 0 埋めする (`src/bt/hid.c:417-426`)。ペア前は 100ms の空 `A1 00 timer` で生かす (`src/bt/hid.c:175-183`, `src/bt/hid.c:129-132`)
- スティック 12bit 化は `pack_stick_12bit` のみが行い (`src/proto/pack.c:31-38`，`src/main.c:994-1008`)、ボタン 3B は `ctrl_pack_btn3` が作る (`src/proto/pack.c:4-29`)。GR/GL/C/Headset・予約bitは輸送位置なしのため落とす (`src/proto/pack.c:25-28`)
...
- `0x00` 正常・`0x01` LEN不正・`0x02` CRC不一致・`0x03` SEQ欠番・`0x04` UNSUPPORTED版・`0x05` UART overrun・`0x06` parser overflow である (`spec/protocol_v3.md:144`, `src/proto/protocol.h:45-53`)
- CONFIG 拒否は `0x10 + (TYPE & 0x0F)` で作る (`src/proto/dispatch.c:25-28`)。すなわち `0x30`→`0x10`・`0x31`→`0x11`・`0x34`→`0x14` であり (`spec/protocol_v3.md:149-161`)、範囲外秒数・未保存再生・範囲外有線値で発火する (`src/proto/dispatch.c:63-94`)
