...
& $cmake --build build
```
Variant configs: `-DPOC_DATA_BAUD=115200` (derated for 115200-bps adapters; default 1Mbps per spec), `-DWIRED_DEFAULT=0` (wireless boot; default 1). Build variants in a temp dir (don't dirty `build/`), copy the UF2 to `log/`.
...
```
Single test: `ctest --test-dir build-host -R <protocol|usb|config> -V`. MSVC needs `/utf-8` (Japanese comments, C4819) — already set in `tests/host/CMakeLists.txt`, don't remove.
